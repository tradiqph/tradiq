...
interface MemberNetworkSheetProps {
  open: boolean;
...
export function MemberNetworkSheet({
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/console/members/${member.id}/network`, {
  ...
  const fetchLevelMembers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/console/members/${member.id}/network?${params}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
      if (!res.ok) {
  ...
  }, [user, member, selectedLevel, query, offset]);
  ...
  useEffect(() => {
    ...
    setOffset(0);
    void fetchSummary();
  }, [open, member, fetchSummary]);

  useEffect(() => {
    if (!open || !member) return;
    void fetchLevelMembers();
  }, [open, member, selectedLevel, query, offset, fetchLevelMembers]);
              ...
              <div className="flex items-center justify-between border-t border-white/5 pt-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    onClick={() => setOffset((current) => current + NETWORK_PAGE_SIZE)}
                    className="cursor-pointer text-xs text-zinc-400 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    Next
                  </button>

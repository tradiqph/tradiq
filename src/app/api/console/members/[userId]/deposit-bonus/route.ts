...
import { creditMemberDepositBonus } from "@/lib/console/admin-deposit-bonus";
import {
...
export async function POST(
  ...
  try {
    ...
    const { bonusId } = await creditMemberDepositBonus(auth.db, {
    ...
    console.info(
      `[console/deposit-bonus] ${auth.decoded.email} credited bonus ${parsed.data.amount} to ${userId} (bonus ${bonusId})`
    );

    return NextResponse.json({ success: true, bonusId });
  } catch (e) {
    ...
    return apiError(
      "console/members deposit-bonus",
      e,
      500,
      "Failed to credit deposit bonus"
    );
  }
}
